...
void a3demo_loadGeometry(a3_DemoState *demoState)
{
	...
	const a3byte *const geometryStream = "./data/gpro26_base_geom_active_2.dat";
	...
	if (streaming && a3fileStreamOpenRead(fileStream, geometryStream))
	{
		// read from stream
	...
	else if (a3fileStreamOpenWrite(fileStream, geometryStream))
	{
		// create new data
		...
		a3proceduralCreateDescriptorCircle(displayShapes + 4, a3geomFlag_wireframe, a3geomAxis_x, 1.0f, 32, 1);
		a3proceduralCreateDescriptorCircle(displayShapes + 5, a3geomFlag_wireframe, a3geomAxis_y, 1.0f, 32, 1);
		a3proceduralCreateDescriptorCircle(displayShapes + 6, a3geomFlag_wireframe, a3geomAxis_z, 1.0f, 32, 1);
		for (i = 0; i < displayShapesCount; ++i)
		{
	...
	currentDrawable = demoState->draw_ring_x;
	...
	currentDrawable = demoState->draw_ring_y;
	...
	currentDrawable = demoState->draw_ring_z;
	sharedVertexStorage += a3geometryGenerateDrawable(currentDrawable, displayShapesData + 6, vao, vbo_ibo, sceneCommonIndexFormat, 0, 0);
...
void a3demo_loadShaders(a3_DemoState *demoState)
{
	...
	a3_SceneShaderProgram *currentDemoProg;
	a3i32 flag;
	a3ui32 i;
	...
	//added for gird drawing
	currentDemoProg = demoState->prog_drawGrid;
...
void a3demo_loadValidate(a3_DemoState* demoState)
{
	...
	a3_SceneShaderProgram* currentProg = demoState->shaderProgram,
	...
	a3_refreshDrawable_internal(demoState->draw_link, currentVAO, currentBuff);
	a3_refreshDrawable_internal(demoState->draw_node, currentVAO, currentBuff);
	a3_refreshDrawable_internal(demoState->draw_ring_x, currentVAO, currentBuff);
	a3_refreshDrawable_internal(demoState->draw_ring_y, currentVAO, currentBuff);
	a3_refreshDrawable_internal(demoState->draw_ring_z, currentVAO, currentBuff);
	...
	a3_refreshDrawable_internal(demoState->draw_character_skin_alt, currentVAO, currentBuff);
